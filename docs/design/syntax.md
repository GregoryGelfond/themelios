...
/// The string door: text and a dialect, admitting the text into a `Source`
/// itself (base §3.2) — the one entry that can refuse before a tree exists,
/// with `TooLarge` when the text exceeds `Source::MAX_LEN` (§13).
...
lexer and no limit. `parse_str` is `parse` over a `Source` the door admits
...
- **One wrapper per node kind** in Appendix A, named by the kind in
  ...
  `Aggregate`, `AggregateElement`, `SetElement`, `DisjunctionElement`,
  `Term`, `TheoryTerm`, and `TheoryDefItem` — each generated by the
  `ast_enum!` macro: an `AstNode` casting on the kind, and a
  `From<Variant>` for each alternative, the natural lift of a wrapper into
  its class. `Program`, `StatementFragment`, and `TermFragment` (§6.1) are
  the roots. Three enums whose alternatives are *not* all nodes —
  `LiteralInner` (`Literal`'s inner form: an atom, a comparison, or a bare
  `#true`/`#false` token), `Constant` (a constant term's: an identifier,
  numeral, or string token, or a bare `#inf`/`#sup`), and `TheoryOpTermItem`
  (an operator token or a theory term) — are hand-written, mixing node and
  typed-token variants, and carry the same `From` on their node and
  typed-token variants only; the placement, and why a bare-token variant
  takes none, are §8.3's.
...
**The hand-written conversions live here.** `AstToken` is itself
hand-written — rowan exports `AstNode` but no token analogue, so the
idiom is hand-rolled as in rust-analyzer (above) — and so are the `From`s
of the three enums §8.1 could not generate, the ones mixing node and
typed-token variants: `LiteralInner` takes an atom or a comparison node,
`Constant` an `Ident`, `NumberLit`, or `StringLit` token, and
`TheoryOpTermItem` a theory term — each `From` written beside its enum.
A bare-token variant (`#true`, `#inf`, an operator) takes **no**
`From<SyntaxToken>`: it would admit a token of any kind unchecked, and two
such variants could not share the one impl.
...
nothing when `a` is *skipped* — `is_skipped(a)`, the predicate for a trivia
token or an empty node (§5.4), exactly what the significant-child walk and
the steps to `prev` and `next` pass over — since the rule never names a
skipped element as an anchor. So `yields exactly`, above, is literal over
*every* element: the nameable anchors carry their comments, and the skipped
ones are empty.
...
/// A single token's content, the per-kind projection above — the rule's one
/// home, so `Comment::content` and `ScriptBody::value` are the same trims.
pub fn content(token: &SyntaxToken) -> &str;
/// That content as a certificate compares it: `content` as it stands under
/// `LayoutOnly`, canonically respelled under `UpToSpelling` (§11.3).
...
`canonical_spelling` is O(1); `content` and `compared` are O(|token text|)
at worst — `content`'s trim, `compared`'s respelling — and O(1) beyond the
text read, `compared` borrowing on the equal path and allocating only for a
synonym respelled under `UpToSpelling`.
...
Every refusal a door here surfaces implements `Display` and
`std::error::Error`, whether this tier owns it or base owns it and this
tier surfaces it. The **syntax-owned** refusals — `InvalidStringLiteral`,
`NotAttachable`, `Mismatch`, and `TokenSourceLawViolation` — implement both,
a `Display` witness in words a formatter's safe mode `?`-composes; the
**base-owned** refusals a syntax door surfaces — `PositionRefusal` at the
token door (§4.3), `SliceRefusal` at `tree::source_text` (§5.3), and
`TooLarge` at the string door `parse_str` (§6.1) — carry base's own
`Display` and `Error` (base §3.2), named here so the enumeration is whole.
`SyntaxKind` and `Dialect` implement `Display`
...
(an empty report is the laws holding); `parse` over an admitted `Source`,
and every general entry point over a token source (§4.3) — which declares
its own text, admitted or not — each yielding a tree (aspif input, unlawful
token sources, and nesting past the bound included, each with its typed
diagnostic; the string door `parse_str`, which admits its text into a
`Source` itself, is the one door that can refuse before a tree exists — the
table); `Parse`'s accessors, `has_errors`,
...
- **Scaling shapes (criterion):** parse linear in text; the certificate
  linear in both texts; bulk attachment linear in the tree; the
  significant-child walk linear in a node's children; bulk attachment, the
  significant-child walk, and the token stream linear in a `%!` doc block,
  and the significant-child walk linear in a *statement-free* `%!` run
  directly under `PROGRAM` — the docs-position shape and its root form
  (§5.4), where a re-scan of a doc line's preceding run would be quadratic;
  the inverse leading form and the directional previous-sibling step linear
  in the doc block they walk back across; the oracle constant per pair; the
...
- **§6.1, §8.1, §8.3, §9.3, §11.1, §12.5, §13, §16** (2026-09-09):
  honesty-only reconciliations bringing the document current with the
  shipped surface. §6.1 and §13 name the string door `parse_str`, the one
  entry that admits its own text and so the one that can refuse (`TooLarge`)
  — §13's totality gloss tightened to §4.3's admitted-`Source`-versus-
  declared-token-source distinction. §8.1 lists the `ast_enum!`-generated
  class enums whole (`SetElement`, `DisjunctionElement`, and `TheoryDefItem`
  had been omitted, `LiteralInner` misgrouped among them) and states the
  generated `From<Variant>`; §8.3 places the hand-written `From`s of the
  three token-bearing enums. §9.3 names `is_skipped` — the trivia-or-empty
  predicate the significant-child walk and the `prev`/`next` steps share —
  so the attachment law reads literally over every element. §11.1 lists the
  per-token `content`/`compared` projections and §12.5 completes the
  refusal-trait roster (`SliceRefusal` and `TooLarge`, base-owned and
