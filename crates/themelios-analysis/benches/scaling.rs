...
//! and the head-cycle-free classification (§6.2) linear in a disjunctive head sharing a
//! positive cycle. As the last surface lands, `Analysis::of` linear in `program + edges`
//! joins them. The test suite asserts the shapes hold (the in-suite scaling tripwires,
//! `tests/scc_laws.rs`, `tests/safe_laws.rs`, `tests/classify_laws.rs`), while these
//! benchmarks measure the absolute numbers across doubling sizes, so a super-linear
//! regression shows as a curve (docs/specification.md §10.2). Run with `cargo bench`.
