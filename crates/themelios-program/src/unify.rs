//! Substitution and fresh names (docs/design/program.md §9.2, §11.1): the one
//! substitution core the unifier produces (§11) and the transformation surface consumes
//! (§9) — the query side binds with it, the transform side rewrites with it. The
//! substitution is **triangular** (a binding's term may mention bound variables), which is
//! what lets the unifier produce it in near-linear space; [`Term::substitute`] is the
//! **resolving** reader that follows the chains to the fixpoint. Construction is
//! crate-internal, so a successful unify is the only public producer of a substitution: the
//! empty substitution means *unified, binding nothing*, and must never be asserted without
//! having unified (§11.2). [`Fresh`] is the collision-free source of new variable and
//! predicate names that rename-apart (§11) and an optimizer's auxiliaries draw from.
...
impl Substitution {
    ...
    // a successful unify. They are test-gated while the unifier (§11) that will produce a
    // substitution through them is still forthcoming, so the shipped build carries no unused
    // door; the in-module laws exercise them meanwhile.
    ...
    /// A substitution over the given variable/binding pairs — the unifier's and the
    /// transform surface's internal construction door (§11.1), keeping the mint restriction.
    #[cfg(test)]
