...
//! (docs/design/program.md §1, §18; docs/specification.md §12.3): the shipped
//! closure is exactly the base and syntax tiers and syntax's own closure,
//! FFI-free, with the build scripts admitted by name and none of this crate's
//! own. What is in the closure is a question about the resolved graph, so it is
//! read from `cargo metadata` — Cargo's own account of it — never from a
//! manifest's text (the reading the base and syntax tiers established).
...
/// The shipped closure, exactly: the two lower tiers and syntax's own closure
/// (docs/design/program.md §1; docs/design/syntax.md §14).
const CLOSURE: [&str; 8] = [
    "themelios-base",
    "themelios-syntax",
    "rowan",
...
#[test]
fn the_shipped_closure_is_exactly_the_two_tiers_and_syntax_closure() {
    ...
    assert_eq!(
        ...
        "docs/design/program.md §1: the shipped closure, exactly"
    );
}
...
fn the_closure_is_ffi_free_with_build_scripts_admitted_by_name() {
    ...
    assert_eq!(
        ...
        "docs/design/program.md §1: the build scripts in the closure are exactly the admitted list"
    );
}
...
fn this_crate_has_no_build_script() {
    ...
    let this = packages
        .values()
        .find(|package| package.name == "themelios-program")
        .expect("this crate is in the graph");
    assert!(
...
fn unsafe_code_is_forbidden_at_the_crate_root() {
    ...
    assert!(
        ...
        "docs/design/program.md §1: forbid, not merely deny, at the root"
    );
}
