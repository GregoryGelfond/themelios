...
//!
//! The walks, no exception (§13, §16): clone, drop, `PartialEq`/`Eq`, `PartialOrd`/`Ord`,
//! `Hash`, `render`, `substitute`, `evaluate`, `canonicalize`, `fold`, and the pre-order
//! traversal — over the four self-recursive families of grammar §10 (the term, its
//! constant-term and value-term subsets, and the theory term), realized here as `Symbol`,
//! `Term`, and `TheoryTerm`. `substitute` is exercised two ways: over a deep term, and
...
/// the same depth overflows. Calibrated, not arbitrary (records `2026-08-28-task-25`): a
